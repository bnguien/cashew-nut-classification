...
# Tên camera (dùng trong tên file ảnh)
CAM_NAME     = "cam01"
...
def index():
    ...
    label_status = (
        f"<b style='color:#f90'>MANUAL: {LABEL_MAP.get(ml, ml)}</b>"
        if ml else
        "<b style='color:#0f9'>AI MODE (random)</b>"
    )
...
.btn{{display:inline-block;padding:10px 20px;border-radius:6px;border:none;font-size:15px;
      font-weight:bold;cursor:pointer;margin:4px;text-decoration:none}}
.w{{background:#2a7d4f;color:#fff}}.b{{background:#7d6e2a;color:#fff}}
.d{{background:#7d2a2a;color:#fff}}.ai{{background:#2a4a7d;color:#fff}}
.sm{{background:#444;color:#eee;font-size:13px}}
...
<div class="card">
  ...
  <div style="font-size:12px;color:#777;margin-top:8px">
    Lưu vào: <code>{BASE_PATH}/{time.strftime('%Y%m%d')}/{mode}/</code>
  </div>
</div>
<div class="card">
  <div style="margin-bottom:10px;font-size:13px;color:#aaa">
    Thống kê ({time.strftime('%Y%m%d')})
  </div>
  <div class="stat">
...
<div class="card">
  <div style="background:#111;padding:10px;border-radius:6px;font-size:13px">
    Kết quả cuối: <b>{last}</b>
  </div>
...
# Tên file: {CAM_NAME}_{label}_{timestamp_ms}.jpg
# Ví dụ:    cam01_whole_1718000000000.jpg
# =============================================================================
@app.route("/upload", methods=["POST"])
def upload_file():
    global ESP32_IP

    # Tự động cập nhật IP ESP32 từ client IP
    client_ip = request.remote_addr
    if client_ip and client_ip != ESP32_IP:
        ESP32_IP = client_ip
        print(f"🔄 Cập nhật IP ESP32: {ESP32_IP}")
    ...
    try:
        ...
        if img is None:
            print("❌ Decode ảnh thất bại")
            return "FAILED_DECODE", 400

        # Xác định nhãn (manual hoặc random)
        if state["manual_label"] is not None:
            ...
            mode_str  = "AI_RANDOM"
            conf      = round(random.uniform(0.55, 0.99), 2)  # random rộng để test cả 2 nhánh

        target_label = LABEL_MAP[ai_result]
        state["count"][ai_result] += 1

        # --- Tên file: {cam}_{label}_{timestamp_ms}.jpg ---
        ts_ms     = int(time.time() * 1000)
        filename  = f"{CAM_NAME}_{target_label}_{ts_ms}.jpg"
        date_str  = time.strftime("%Y%m%d")
        save_dir  = os.path.join(BASE_PATH, date_str, state["mode"], target_label)
        os.makedirs(save_dir, exist_ok=True)  # Đảm bảo folder tồn tại
        save_path = os.path.join(save_dir, filename)

        cv2.imwrite(save_path, img)

        state["last_result"] = f"{ai_result} ({target_label}) [{mode_str}] conf:{conf:.2f}"
        print(f"📸 [{mode_str}] class={ai_result} conf={conf:.2f} → {save_path}")
        ...
        if DISPLAY_GUI:
            try:
                ...
                cv2.putText(
                    d,
                    f"[{mode_str}] {ai_result}:{target_label} {conf:.2f}",
        ...
        # Trả về JSON: {"c": class_int, "f": confidence_float}
        response_payload = f'{{"c":{ai_result},"f":{conf:.2f}}}'
