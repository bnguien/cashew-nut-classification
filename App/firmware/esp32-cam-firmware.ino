/**
 * ESP32-CAM (AI Thinker) — Cashew upload + MQTT + Serial1 <-> Arduino
 *
 * Copy file này vào Arduino IDE (hoặc đè file trong Downloads).
 * Bật Serial Monitor 115200 để xem log debug giao tiếp Arduino.
 */
...
// =============================================================================
// DEBUG — bật/tắt log chi tiết Serial1 (Arduino)
// =============================================================================
...
// =============================================================================
const char* WIFI_SSID = "OYE TRA SUA T1";
const char* WIFI_PASSWORD = "39393939";

const char* SERVER_HOST = "192.168.1.107";
...
// MQTT
// =============================================================================
const char* MQTT_HOST = "192.168.1.107";
...
void sendResultToArduino(const String& result) {
  countCmd++;
  logLine("ARD_TX", "cmd#%d -> Serial1 line: \"%s\" (len=%d)", countCmd, result.c_str(), result.length());
  Serial1.println(result);
#if DEBUG_ARDUINO_SERIAL1
  logLine("ARD_TX", "Arduino expects single char line: 0,1,2,3 (newline terminated)");
#endif
}
...
void publishStatus(const char* severity, bool isFault, const String& message) {
  ...
  bool ok = mqttClient.publish(TOPIC_STATUS, out, n, false);
...
void publishHeartbeat() {
  ...
  mqttClient.publish(TOPIC_HEARTBEAT, out, n, false);
...
bool postImageMultipart(camera_fb_t* fb, String& responseBody, int& httpCode) {
  ...
  String req = "POST " + String(SERVER_PATH) + " HTTP/1.1\r\n";
  req += "Host: " + String(SERVER_HOST) + ":" + String(SERVER_PORT) + "\r\n";
  req += "Connection: close\r\n";
  req += "Content-Type: multipart/form-data; boundary=" + boundary + "\r\n";
  if (strlen(ESP_API_KEY) > 0) req += "X-ESP-Key: " + String(ESP_API_KEY) + "\r\n";
  ...
  size_t sent = 0;
  while (sent < fb->len) {
    size_t chunk = min((size_t)1024, fb->len - sent);
    client.write(fb->buf + sent, chunk);
    sent += chunk;
  }
  client.print(tail);
  logLine("HTTP", "uploaded JPEG bytes=%u multipart_total=%u", (unsigned)fb->len, (unsigned)totalLen);
  ...
  statusLine.trim();
  httpCode = 0;
  if (statusLine.startsWith("HTTP/1.1 ")) httpCode = statusLine.substring(9, 12).toInt();
  logLine("HTTP", "status: %s -> code=%d", statusLine.c_str(), httpCode);
  ...
  responseBody = "";
  while (client.available()) responseBody += client.readString();
  client.stop();

#if DEBUG_LOG_HTTP_BODY
  if (responseBody.length() > 0) {
    size_t show = responseBody.length() > 240 ? 240 : responseBody.length();
    Serial.printf("[%10lu][%-10s] body (first %u chars): ", millis(), "HTTP", (unsigned)show);
    Serial.write((const uint8_t*)responseBody.c_str(), show);
    if (responseBody.length() > 240) Serial.print("...");
    Serial.println();
  } else {
    logLine("HTTP", "body empty");
  }
#endif
  return true;
}
...
void syncWithArduino() {
  ...
  logLine("ARD_RX", "line len=%d text=\"%s\"", cmd.length(), cmd.c_str());
#if DEBUG_ARDUINO_SERIAL1
  logHexPreview("ARD_RX", (const uint8_t*)cmd.c_str(), cmd.length(), 32);
#endif
  if (cmd[0] == '1') {
    int flashValue = 20;
    ...
    logLine("ARD_RX", "unknown first char='%c' (expected '1' for capture)", cmd[0]);
  }
}
